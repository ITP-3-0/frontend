...
import { Search, BellRing, AlertTriangle, Calendar, Clock, CheckCircle, RefreshCw, ExternalLink, ArrowLeft } from "lucide-react";
...
export default function NotificationsView() {
    ...
    const markAsRead = async (notificationId) => {
        try {
            // Optimistic update
            const updatedNotifications = notifications.map((notif) => (notif._id === notificationId ? { ...notif, isRead: true } : notif));
    ...
    const filteredNotifications = notifications.filter((notification) => {
        ...
        const matchesStatus =
            statusFilter === "all" || (statusFilter === "read" && notification.isRead) || (statusFilter === "unread" && !notification.isRead);
                ...
                <div className="@container/main flex flex-1 flex-col gap-2 py-8">
                    <div className="flex flex-col gap-4 md:gap-6 px-4 lg:px-6 max-w-5xl mx-auto w-full">
                        <div className="flex items-center gap-2 mb-2">
                            <ArrowLeft className="h-6 w-6 text-black dark:text-slate-500 cursor-pointer" onClick={() => window.history.back()} />
                            <BellRing className="h-6 w-6 text-black dark:text-slate-500" />
                            <h1 className="text-2xl font-bold">My Notifications</h1>
                            {unreadCount > 0 && <Badge className="ml-2 bg-black text-white dark:bg-slate-600">{unreadCount} unread</Badge>}
                        </div>
                            ...
                            <CardHeader className="bg-slate-100/50 dark:bg-slate-800/10 rounded-t-lg border-b">
                                <div className="flex items-start justify-between">
                                    <div>
                                        ...
                                        <CardDescription className="mt-1">View all your system and communication notifications</CardDescription>
                                    </div>

                                    <Button variant="outline" size="sm" disabled={unreadCount === 0} onClick={markAllAsRead} className="text-sm">
